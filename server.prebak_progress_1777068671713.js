...
async function loadDayData(date) {
  ...
  if (all.length > 0) if (all.length > 0) dataCache.set(date, all); // 0건이면 캐시 저장 안 함 (나중에 데이터 올라오면 재조회) // 0건이면 캐시 저장 안 함 (나중에 데이터 올라오면 재조회)
...
async function handleCommand(text, chatId) {
  ...
  if (raw === '/공급가업데이트' || raw === '/가격업데이트') {
    (async () => {
      try {
        const { run } = require('./auto_price');
        await run();
      } catch(e) {
        const tgBody = new URLSearchParams({
          chat_id: process.env.TG_CHAT_ID,
          text: '❌ 공급가 업데이트 오류: ' + e.message,
          parse_mode: 'HTML'
        });
...
async function pollTelegram() {
  ...
  try {
    ...
    for (const update of data.result) {
      ...
      if (!msg.text.startsWith('/') && !msg.text.includes('시세')) continue;
