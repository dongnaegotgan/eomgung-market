...
async function sendTrackedItems() {
  ...
  for (const target of tracked) {
    ...
    try {
      ...
      await fetch(`https://api.telegram.org/bot${process.env.TG_TOKEN}/sendMessage`,{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:tgBody.toString()});
