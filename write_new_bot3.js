/**
 * write_new_bot3.js
 * tg_status_bot.js v11
 * - 명령어 통합: /주문건확인, /주문출력, /승인상태, /명령어리스트
 * - 곳간 상품 파싱: DOM evaluate → HTML 문자열 정규식으로 변경
 */
const fs = require('fs');
const path = require('path');

const TARGET = path.join(__dirname, 'tg_status_bot.js');
const bak = TARGET.replace('.js', `.v10bak_${Date.now()}.js`);
...
async function adminLogin() {
  ...
  for (let hop = 0; hop < 6; hop++) {
    const res = await req(opts, nextBody);
    Object.assign(cookies, parseCookies(res.headers['set-cookie']));
    const loc = res.headers['location'];
    if (!loc) break;
    let h = ADMIN_HOST, p = loc;
    if (loc.startsWith('http')) {
      try { const u = new URL(loc); h = u.hostname; p = u.pathname + u.search; } catch(e) {}
    }
    opts = { hostname: h, path: p, method: 'GET',
      headers: { 'Cookie': cookieStr(cookies), 'User-Agent': 'Mozilla/5.0', 'Referer': ADMIN_URL } };
    nextBody = null;
  }
  adminCookies = cookies;
  adminLoginTime = Date.now();
  log(\`  [어드민] 로그인, 쿠키: \${Object.keys(cookies).join(', ') || '없음'}\`);
}
...
async function getGotganOrders() {
  ...
  try {
    ...
    log(\`  [곳간] 페이지 로드 완료, HTML 길이: \${html.length}\`);
    ...
    // ★ 상품 파싱: HTML 문자열에서 직접 정규식으로 추출
    // TD 태그 안에서 "상품명 / N 개" 패턴 찾기
    const prodMap = {};
    let totalAmount = 0;
    ...
    while ((tdMatch = tdPattern.exec(html)) !== null) {
      ...
      if (raw.length > 3) tdContents.push(raw);
    }
    log(\`  [곳간] TD 수: \${tdContents.length}\`);

    // "상품명 / N 개" 패턴이 있는 TD 찾기
    for (const td of tdContents) {
      const m = td.match(/^(.+?)\\s*\\/\\s*(\\d+)\\s*개/);
      if (!m) continue;
      ...
      if (name.length < 2 || qty <= 0 || qty >= 1000) continue;
      if (/배송비|배송|주소|취소|결제|입금|운송/.test(name)) continue;
      if (/^\\d+$/.test(name)) continue;

      log(\`  [곳간 상품발견] "\${name}" / \${qty}개 | 원문: "\${td.slice(0,80)}"\`);

      if (!prodMap[name]) prodMap[name] = { qty: 0, amt: 0 };
      prodMap[name].qty += qty;
    }

    log(\`  [곳간] 상품 \${Object.keys(prodMap).length}종, 총액 \${totalAmount.toLocaleString()}원\`);
...
async function handleCommand(cmd, chatId) {
  ...
  if (cmd === '/주문출력' || cmd === '주문출력') {
    await tgSend('📥 곳간 주문 출력 시작... (약 30초 소요)', chatId);
    (async () => {
      let dlBrowser = null;
      try {
        const os = require('os');
        ...
        await sleep(2000);
        await dlPage.goto(
          \`\${FG_BASE}/NewOrder/ExcelDownload?fileName=\${encodeURIComponent(fileName)}\`,
...
console.log('✅ tg_status_bot.js v11 작성 완료!');
console.log('\n실행:');
