/**
 * write_new_bot4.js
 * tg_status_bot.js v12
 * 곳간 상품 파싱: HTML 문자열 → evaluate() DOM cells[5] 방식으로 수정
 */
const fs = require('fs');
const path = require('path');

const TARGET = path.join(__dirname, 'tg_status_bot.js');
const bak = TARGET.replace('.js', `.v11bak_${Date.now()}.js`);
 ...
 * 곳간 파싱: evaluate() DOM cells[5] 방식 (debug_orders.js 검증 완료)
 */
...
async function getGotganOrders() {
  ...
  try {
    ...
    // ★ 상품 파싱: evaluate() DOM 직접 접근 - debug_orders.js 검증 완료
    const result = await fgPage.evaluate(() => {
      ...
      rows.forEach((row, ri) => {
        ...
        // debug에서 확인: "상품명 / N 개 · 선택:옵션..."
        const m = prodCell.match(/^(.+?)\s*\/\s*(\d+)\s*개/);
        if (!m) return;
        ...
        if (/배송비|배송|주소|취소|결제|입금/.test(name)) return;

        // 금액 (cells[6])
        const amtCell = cells[6] || '';
        let amt = 0;
        const amtMs = amtCell.match(/(\d{1,3}(?:,\d{3})+|\d{4,8})\s*원/g);
        if (amtMs) amtMs.forEach(a => {
          const v = parseInt(a.replace(/[,원]/g,''));
    ...
    log(\`  [곳간 디버그 cells] \${result.debugCells.slice(0,7).join(' | ')}\`);
...
console.log('✅ tg_status_bot.js v12 작성 완료!');
console.log('\n실행:');
