/**
 * write_new_bot8.js
 * tg_status_bot.js v16 생성
 * 핵심 수정:
 *   - 어드민: form target=_self → waitForNavigation 로그인 성공
 *   - 상태 메시지: "조회 중.." → 진행상황 표시 개선
 *   - 곳간: split("/") 파싱 유지
 *   - tgSend: 재시도 3회
 */
const fs = require('fs');
...
const BAK = TARGET + '.v15bak_' + Date.now() + '.js';
...
console.log('');
console.log('✅ tg_status_bot.js v16 작성 완료!');
console.log('');
console.log('변경사항:');
console.log('  ① 어드민 로그인: form.target=_self + waitForNavigation');
console.log('  ② 어드민 로그인: form.onsubmit 제거 (frmCheck 우회)');
console.log('  ③ 상태메시지: "조회 중.." → "주문 현황 조회 중..." (도매몰/동네곳간 표시)');
console.log('');
console.log('실행:');
console.log('   node write_new_bot8.js');
