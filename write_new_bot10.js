...
/**
 * write_new_bot10.js
 * 실행: node write_new_bot10.js
 */
const fs   = require('fs');
const path = require('path');

const TARGET = path.join(__dirname, 'tg_status_bot.js');
const BAK    = TARGET + '.v17bak_' + Date.now() + '.js';
    ...
    (async function() {
      ...
      try {
        ...
        if (respR) {
          ...
          var mm = text2.match(/order_\d+\.xlsx/);
          if (mm) fileName = mm[0];
...
console.log('');
console.log('✅ tg_status_bot.js v18 작성 완료!');
console.log('   핵심 변경: 주문 페이지 navigate + waitForResponse XHR 인터셉트');
console.log('   어드민 브라우저가 직접 XHR 트리거 → 쿠키 100% 자동 포함');
console.log('');
console.log('다음 명령어:');
