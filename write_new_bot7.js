/**
 * write_new_bot7.js
 * tg_status_bot.js v15 생성
 * 핵심 수정:
 *   - 어드민 로그인: admid/admpwd evaluate 직접 입력
 *   - hiddenFrm target 대응 (쿠키 감시 방식)
 *   - 어드민 도매몰 / 동네곳간 이름
 *   - tgSend 재시도 3회
 *   - 곳간 split('/') 파싱 유지
 */
...
const BAK = TARGET + '.v14bak_' + Date.now() + '.js';
if (fs.existsSync(TARGET)) { fs.copyFileSync(TARGET, BAK); console.log('백업:', path.basename(BAK)); }

const code = `/**
 * tg_status_bot.js v15
 * 명령어: /주문건확인 /주문출력 /승인상태 /명령어리스트
 * 어드민: Puppeteer (admid/admpwd evaluate 직접입력, hiddenFrm 쿠키감시)
 * 곳간: Puppeteer split("/") 파싱
 */
...
// ── 어드민 로그인 (admid/admpwd evaluate, hiddenFrm 쿠키감시) ────────────────
async function adminLogin() {
  ...
  // evaluate로 admid/admpwd 직접 입력
  await adminPage.evaluate((uid, upw) => {
    document.querySelectorAll('input').forEach(el => {
      const n = (el.name || '').toLowerCase();
      if (n === 'admid' || n === 'id' || n === 'admin_id') {
        el.value = uid;
        el.dispatchEvent(new Event('input', { bubbles: true }));
        el.dispatchEvent(new Event('change', { bubbles: true }));
      }
      if (n === 'admpwd' || n === 'pw' || n === 'password' || n === 'admin_pw') {
        el.value = upw;
        el.dispatchEvent(new Event('input', { bubbles: true }));
        el.dispatchEvent(new Event('change', { bubbles: true }));
      }
  ...
  log('  [어드민] evaluate 직접 입력 완료 (admid/admpwd)');
  await sleep(300);

  // 로그인 버튼 클릭 (hiddenFrm target → navigate 없음)
  ...
  if (submitBtn) {
    ...
    await adminPage.evaluate(() => {
      const form = document.querySelector('form#signup') || document.querySelector('form[name="signup"]') || document.querySelector('form');
      if (form) form.submit();
    });
  }

  // hiddenFrm 처리 대기 (navigate 안 일어남 → 쿠키 or URL 감시)
  await sleep(2000);
  for (let i = 0; i < 5; i++) {
    const url = adminPage.url();
    const cookies = await adminPage.cookies();
    const hasSession = cookies.some(c => /sess|admin|login/i.test(c.name));
    log(\`  [어드민] 대기[\${i}] 쿠키수: \${cookies.length}, URL: \${url.slice(0,50)}\`);
    if (hasSession || !url.includes('login')) break;
    await sleep(1000);
  }
...
async function getAdminOrders() {
  ...
  try {
    ...
    log(\`  [어드민 stats] 신규(spnNew5)=\${stats.spnNew5}, 오늘전체(spnNew8)=\${stats.spnNew8}\`);
...
console.log('✅ tg_status_bot.js v15 작성 완료!');
console.log('   - 어드민: admid/admpwd evaluate 직접 입력');
console.log('   - 어드민: hiddenFrm 처리 (쿠키 감시)');
console.log('   - 이름: 어드민 도매몰 / 동네곳간');
