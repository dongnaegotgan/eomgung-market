...
function handleCommand(cmd,chatId){
  ...
  if(_hc){
    var _wm=cmd.match(/(\d[\d,]+)\uC6D0/);
    var _np=_wm?parseInt(_wm[1].replace(/,/g,'')):0;
    var _pn=cmd.replace(/(\d[\d,]+)\uC6D0.*/,'').replace(/\uC73C\uB85C|\uB85C|\uBCC0\uACBD|\uC218\uC815|\uBC14\uAFB8|\uB2E8\uAC00|\uD574\uC918|\uC8FC\uC138\uC694/g,'').trim();
    if(_np>=100&&_np<10000000&&_pn.length>=2){
      tgSend('\u23F3 '+_pn+' '+_np.toLocaleString()+'\uC6D0 \uCC98\uB9AC\uC911...',chatId);
      try{var _ap=require('./auto_price_single');_ap.run(_pn,_np).then(function(r){if(r.success){var _ns=(r.updatedNames||[_pn]).join(', ');tgSend('\u2705 \uB2E8\uAC00 \uBCC0\uACBD \uC644\uB8CC\n'+_ns+'\n'+(r.oldPrice||0).toLocaleString()+'\uC6D0 -> '+_np.toLocaleString()+'\uC6D0\n\uC5B4\uB4DC\uBBFC \uC5C5\uB85C\uB4DC \uC644\uB8CC',chatId);}else if(r.needsSelection){var _ls=r.items.map(function(it,i){return (i+1)+'. '+it.name;}).join('\n');tgSend('\uD83D\uDD0D "'+_pn+'" \uC0C1\uD488\uC774 \uC5EC\uB7EC\uAC1C\uC785\uB2C4\uB2E4.\n\uC815\uD655\uD55C \uC0C1\uD488\uBA85\uC73C\uB85C \uB2E4\uC2DC \uC785\uB825\uD574\uC8FC\uC138\uC694:\n'+_ls,chatId);}else{tgSend('\u274C \uBCC0\uACBD \uC2E4\uD328: '+(r.message||''),chatId);}}).catch(function(e){tgSend('\u274C \uC624\uB958:'+e.message,chatId);});}catch(e2){tgSend('\u274C \uBAA8\uB4C8\uC624\uB958:'+e2.message,chatId);}
      return;
    }
  }
...
function poll(){
  ...
  tgGetUpdates(lastUpdateId).then(function(updates){
    updates.forEach(function(u){
      ...
      log('recv: "'+text+'"');
      if(CMDS.indexOf(text)>=0||/변경|수정|바꾸|단가변경/.test(text)) handleCommand(text,chatId);
    });
